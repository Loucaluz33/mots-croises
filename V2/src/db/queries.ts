/**
 * Port of webapp/db.js — all SQLite queries for the Verbicruciste.
...
export function getEnabledSources(): string[] {
  return queryAll<{ source: string }>('SELECT source FROM dict_settings WHERE enabled = 1').map(r => r.source)
}
...
export function searchByPatternGrouped(pattern: string, onlySources: string[] | null = null): Record<string, SearchResult[]> {
  ...
  if (enabled.has('lexique')) {
    ...
    const rows = queryAll<{ ortho: string; ortho_upper: string; lemme: string; cgram: string }>(sql, params)
    const lexique: SearchResult[] = []
    for (const row of rows) {
  ...
  if (extSources.length > 0) {
    ...
    const rows = queryAll<{ mot: string; mot_grid: string; definition: string; categorie: string; source: string }>(sql, params)
    for (const row of rows) {
...
export function loadGrid(nom: string): { id: number; nom: string; json_data: unknown; terminee: number; date_creation: string; date_modif: string } | null {
  const row = queryOne<{ id: number; nom: string; json_data: string; terminee: number; date_creation: string; date_modif: string }>(
...
export function listGrids(): { id: number; nom: string; terminee: number; date_creation: string; date_modif: string }[] {
  return queryAll('SELECT id, nom, terminee, date_creation, date_modif FROM grilles ORDER BY date_modif DESC')
}

export function listGridsFull(): { id: number; nom: string; json_data: string; terminee: number; date_creation: string; date_modif: string }[] {
  return queryAll('SELECT id, nom, json_data, terminee, date_creation, date_modif FROM grilles ORDER BY date_modif DESC')
}
