...
export interface GridEditorState {
  ...
  currentGridAuthor: string
  currentGridDifficulte: string
  currentGridThemes: string[]
  currentGridNomOnline: string
...
export function useGridEditor() {
  const [state, setState] = useState<GridEditorState>(() => {
    ...
    return {
      ...
      currentGridName: '',
      currentGridAuthor: '',
      currentGridDifficulte: '',
      currentGridThemes: [],
      currentGridNomOnline: '',
      dottedFirst: null,
      previewMap: null,
  ...
  // Use a ref to access gridData without re-creating the callback
  const gridDataRef = useRef(state.gridData)
  gridDataRef.current = state.gridData

  const saveUndoState = useCallback(() => {
    const snapshot = gridDataRef.current.map(row =>
      row.map(cell => ({
        ...cell,
  ...
  const createGrid = useCallback((rows: number, cols: number) => {
    ...
    update({
      ...
      modified: false, currentGridId: null, currentGridName: '', currentGridAuthor: '',
      currentGridDifficulte: '', currentGridThemes: [], currentGridNomOnline: '',
  ...
  const setLetter = useCallback((r: number, c: number, letter: string) => {
    saveUndoState()
    setState(prev => {
      // Only clone the affected row for performance
      const newGrid = prev.gridData.map((row, ri) =>
        ri === r ? row.map(cell => ({ ...cell, dotted: cell.dotted ? { ...cell.dotted } : null })) : row
      )
  ...
  const deleteLetter = useCallback((backspace: boolean) => {
    ...
    setState(prev => {
      ...
      if (backspace && !prev.gridData[r][c].letter) {
        // Retreat cursor
        let [nr, nc] = [r, c]
        if (prev.direction === 'across') {
          nc--
          while (nc >= 0 && prev.gridData[nr][nc].black) nc--
          if (nc < 0) return prev
        } else {
          nr--
          while (nr >= 0 && prev.gridData[nr][nc].black) nr--
          if (nr < 0) return prev
        }
        // Only clone the affected row
        const newGrid = prev.gridData.map((row, ri) =>
          ri === nr ? row.map(cell => ({ ...cell, dotted: cell.dotted ? { ...cell.dotted } : null })) : row
        )
        newGrid[nr][nc].letter = ''
        const highlighted = getWordCells(newGrid, nr, nc, prev.direction, prev.rows, prev.cols)
        return { ...prev, gridData: newGrid, selected: [nr, nc], highlightedCells: highlighted, modified: true }
      } else {
        // Only clone the affected row
        const newGrid = prev.gridData.map((row, ri) =>
          ri === r ? row.map(cell => ({ ...cell, dotted: cell.dotted ? { ...cell.dotted } : null })) : row
        ...
        newGrid[r][c].letter = ''
        return { ...prev, gridData: newGrid, modified: true }
      }
    })
  }, [saveUndoState])
  ...
  const previewWord = useCallback((word: string) => {
    setState(prev => {
      if (!prev.selected || prev.lockedPreviewWord) return prev
      const cells = getCellsFromSelected(prev.gridData, prev.selected[0], prev.selected[1], prev.direction, prev.rows, prev.cols)
      const upper = word.toUpperCase()
      const map = new Map<number, string>()
      for (let i = 0; i < Math.min(upper.length, cells.length); i++) {
        const [cr, cc] = cells[i]
        if (!prev.gridData[cr][cc].letter) {
          map.set(cr * prev.cols + cc, upper[i])
        }
      }
      return { ...prev, previewMap: map }
    })
  }, [])

  const clearPreview = useCallback(() => {
    setState(prev => {
      if (prev.lockedPreviewWord || !prev.previewMap) return prev
      return { ...prev, previewMap: null }
    })
  }, [])

  const lockPreview = useCallback((word: string) => {
    setState(prev => {
      if (!prev.selected) return prev
      const cells = getCellsFromSelected(prev.gridData, prev.selected[0], prev.selected[1], prev.direction, prev.rows, prev.cols)
      const upper = word.toUpperCase()
      ...
      return { ...prev, previewMap: map, lockedPreviewWord: upper }
    })
  }, [])

  const unlockPreview = useCallback(() => {
    setState(prev => {
      if (!prev.previewMap && !prev.lockedPreviewWord) return prev
      return { ...prev, previewMap: null, lockedPreviewWord: null }
    })
  }, [])
