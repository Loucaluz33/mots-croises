...
export const dictManagementApi = {
  settings: async () => {
    const dbSettings = db.getDictSettings()
    ...
    // Personal dicts from personal_dicts table
    const personalDicts = db.getPersonalDicts()
    for (const pd of personalDicts) {
      const source = `personnel_${pd.id}`
      const row = dbSettings[source]
      settings.push({
        source,
        name: pd.name,
        type: 'personal',
    ...
    // External dicts from EXTERNAL_DICTS constant
    for (const [key, meta] of Object.entries(db.EXTERNAL_DICTS)) {
      const row = dbSettings[key]
      settings.push({
        source: key,
        name: meta.label,
  ...
  stats: async () => {
    ...
    const stats = sources.map(s => ({ source: s.id, name: s.label, word_count: s.count }))
    return { stats }
  },
  groups: async () => {
