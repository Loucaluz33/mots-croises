...
export function DictEditorTab() {
  ...
  const handleExport = useCallback(async () => {
    if (!selectedDictId) return
    const res = await dictionariesApi.getWords(selectedDictId, 'limit=999999')
    const data = (res.words as DictWord[]).map(({ mot, definitions, categorie, notes }) => ({
      mot,
      definitions,
      categorie,
      notes,
    ...
    const filename = `${dict?.name ?? 'dictionnaire'}.json`
  ...
  }, [selectedDictId, dictionaries])

  // --- Import JSON ---
  const handleImportFile = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0]
    if (!file || !selectedDictId) return
    e.target.value = ''
    ...
    try {
      ...
      for (const entry of data) {
        if (!entry.mot) continue
        const defs = Array.isArray(entry.definitions)
          ? entry.definitions
          : typeof entry.definitions === 'string'
          ? entry.definitions.split('\n').filter(Boolean)
          : []
        try {
          await dictionariesApi.addWord(selectedDictId, {
            mot: entry.mot,
            definitions: defs,
      ...
      setImportStatus(`${imported} mot${imported !== 1 ? 's' : ''} importé${imported !== 1 ? 's' : ''}${skipped > 0 ? `, ${skipped} ignoré${skipped !== 1 ? 's' : ''} (doublons)` : ''}`)
      setTimeout(() => setImportStatus(null), 4000)
      if (imported > 0) markDirty()
      loadWords()
    } catch (err) {
      setImportStatus(`Erreur : ${err instanceof Error ? err.message : 'fichier invalide'}`)
      setTimeout(() => setImportStatus(null), 4000)
    }
  }, [selectedDictId, loadWords])
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <Button variant="outline" onClick={handleExport} disabled={!selectedDictId}>
          <Download className="h-4 w-4" data-icon="inline-start" />
          Exporter JSON
        </Button>

        <Button variant="outline" onClick={() => importInputRef.current?.click()} disabled={!selectedDictId}>
