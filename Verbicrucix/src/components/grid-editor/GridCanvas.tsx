...
export const GridCanvas = memo(function GridCanvas({
  ...
  // Pre-compute highlighted cells as a Set for O(1) lookup instead of O(n) .some()
  const highlightedSet = useMemo(() => {
    const set = new Set<number>()
    for (const [hr, hc] of highlightedCells) {
      set.add(hr * 1000 + hc)
    }
    return set
  }, [highlightedCells])
  ...
  const render = useCallback(() => {
    ...
    for (let r = 0; r < rows; r++) {
      for (let c = 0; c < cols; c++) {
        ...
        } else if (highlightedSet.has(r * 1000 + c)) {
        ...
        if (!cell.black) {
          ...
          // Letter or preview
          const preview = previewMap?.get(r * cols + c)
          if (cell.letter) {
  ...
  }, [rows, cols, gridData, selected, highlightedSet, numberingStyle, rowNumbering, colNumbering, dottedFirst, previewMap, lockedPreviewWord, offset, canvasW, canvasH])
